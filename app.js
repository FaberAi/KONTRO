...
async function doRegister() {
  ...
  if (data.user) {
    ...
    if (!bizErr && biz) {
      ...
      await supabase.rpc('create_default_categories', { p_business_id: biz.id });
    }
  }
...
function showView(name) {
  ...
  const titles = { dashboard: 'Dashboard', nuovo: 'Nuovo movimento', movimenti: 'Movimenti', report: 'Report', sedi: 'Sedi' };
...
async function loadDashboard() {
  ...
  let query = supabase.from('cash_entries').select('*')
    .eq('business_id', currentBusiness.id);
