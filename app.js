...
window.addEventListener('unhandledrejection', e => {
  ...
  if (msg && !msg.includes('AbortError') && typeof showToast !== 'undefined') {
    showToast('Errore: ' + msg.substring(0, 80), 'error');
  }
});
// ============================================
...
async function calcolaDataOttimaleAssegno(importoAssegno, dataConsigliataEl) {
  ...
  try {
    const [
      { data: vers }, { data: movUsc },
      { data: assegniAperti }, { data: ridAttivi },
      { data: fattureAperte }, { data: noteStorico }
    ] = await Promise.all([
      ...
      db.from('assegni').select('importo,data_scadenza,beneficiario').eq('business_id', currentBusiness.id).in('stato',['emesso','da_addebitare']).lte('data_scadenza', in90str),
      db.from('rid_bancari').select('importo,prossimo_addebito,frequenza,nome').eq('business_id', currentBusiness.id).eq('attivo',true),
      db.from('fatture_fornitori').select('importo_totale,data_scadenza,fornitori(ragione_sociale)').eq('business_id', currentBusiness.id).in('stato',['aperta','pagata_parziale']).lte('data_scadenza', in90str),
    ...
    // ── Media incasso/uscite per giorno settimana ─────────────────────
    ...
    // ── Mappa eventi per data {importo, etichette[]} ──────────────────
    const eventiPerData = {}; // { 'YYYY-MM-DD': { uscita: num, etichette: [] } }
    const addEvento = (data, imp, label) => {
      if (!eventiPerData[data]) eventiPerData[data] = { uscita: 0, etichette: [] };
      eventiPerData[data].uscita += imp;
      if (label) eventiPerData[data].etichette.push(label);
    };

    (assegniAperti||[]).forEach(a => {
      if (a.data_scadenza >= today) addEvento(a.data_scadenza, Number(a.importo||0), '📝 Assegno ' + (a.beneficiario||''));
    });
    (fattureAperte||[]).forEach(f => {
      if (f.data_scadenza && f.data_scadenza >= today) addEvento(f.data_scadenza, Number(f.importo_totale||0), '🧾 ' + (f.fornitori?.ragione_sociale||'Fattura'));
    });
    (ridAttivi||[]).forEach(r => {
      let d = r.prossimo_addebito;
      if (!d || d < today) return;
      const freqG = { mensile:30, bimestrale:60, trimestrale:90, semestrale:180, annuale:365 }[r.frequenza] || 30;
      while (d <= in90str) {
        addEvento(d, Number(r.importo||0), '🔄 ' + r.nome);
        const nd = new Date(d + 'T12:00:00'); nd.setDate(nd.getDate() + freqG);
        d = nd.toISOString().split('T')[0];
      }
    });

    // ── Simulazione: costruisce mappa saldo per ogni giorno ───────────
    const BUFFER = importoAssegno * 0.1;
    const saldoPerData = {};
    let saldo = saldoAttuale;
    let primoGiornoOk = null;
    ...
    for (let i = 1; i <= GIORNI; i++) {
      ...
      const usciteProg = eventiPerData[ds]?.uscita || 0;
      saldo += entrate - usciteOp - usciteProg;
      saldoPerData[ds] = { saldo, entrate, usciteOp, usciteProg, eventi: eventiPerData[ds]?.etichette || [] };
      if (!primoGiornoOk && saldo >= importoAssegno + BUFFER) primoGiornoOk = ds;
    }
    ...
    if (primoGiornoOk) {
      const naScadenza = document.getElementById('na-scadenza');
      if (naScadenza && !naScadenza.value) naScadenza.value = primoGiornoOk;
    }
...
// Seleziona una data dal calendario e la imposta nel campo scadenza
function selezionaDataScadenzaAssegno(dataStr) {
...
async function exportEstrattoBancaPDF() {
  ...
  // Legge i filtri dalla tab estratto conto
  const bancaId = document.getElementById('eb-banca')?.value;
  const from = document.getElementById('eb-from')?.value;
  const to   = document.getElementById('eb-to')?.value;

  if (!bancaId) { showToast('Seleziona una banca prima di esportare', 'error'); return; }

  showToast('Generazione PDF...', 'success');

  // Carica dati banca
  const { data: banca } = await db.from('banche').select('*').eq('id', bancaId).single();

  // Carica movimenti dal DB
  let qMov = db.from('movimenti_banca')
    .select('*').eq('banca_id', bancaId).order('data', { ascending: true });
  if (from) qMov = qMov.gte('data', from);
  if (to)   qMov = qMov.lte('data', to);
  const { data: movimenti } = await qMov;

  // Carica versamenti
  let qVers = db.from('versamenti')
    .select('*').eq('business_id', currentBusiness.id).order('data', { ascending: true });
  if (from) qVers = qVers.gte('data', from);
  if (to)   qVers = qVers.lte('data', to);
  const { data: versamenti } = await qVers;

  // Carica assegni addebitati
  let qAss = db.from('assegni')
    .select('*, fornitori(ragione_sociale)')
    .eq('business_id', currentBusiness.id)
    .eq('stato', 'addebitato').order('data_incasso', { ascending: true });
  if (from) qAss = qAss.gte('data_incasso', from);
  if (to)   qAss = qAss.lte('data_incasso', to);
  const { data: assegni } = await qAss;

  // Costruisci lista movimenti unificata
  const allMov = [
    ...(movimenti||[]).map(m => ({
      data: m.data, tipo: m.tipo_movimento || 'Movimento',
      desc: m.descrizione || '—',
      importo: Math.abs(parseFloat(m.importo)||0),
      segno: (parseFloat(m.importo)||0) >= 0 ? 'avere' : 'dare'
    })),
    ...(versamenti||[]).map(v => ({
      data: v.data, tipo: 'Versamento',
      desc: v.note || 'Versamento cassa',
      importo: parseFloat(v.importo)||0,
      segno: 'avere'
    })),
    ...(assegni||[]).map(a => ({
      data: a.data_incasso, tipo: 'Assegno',
      desc: (a.fornitori?.ragione_sociale || '') + (a.numero ? ' N° '+a.numero : ''),
  ...
  const totEntrate = allMov.filter(m=>m.segno==='avere').reduce((s,m)=>s+m.importo,0);
  const totUscite  = allMov.filter(m=>m.segno==='dare').reduce((s,m)=>s+m.importo,0);
  const saldoIniziale = parseFloat(banca?.saldo_iniziale||0);
  const saldoFinale = saldoIniziale + totEntrate - totUscite;
  ...
  doc.text((banca?.nome || 'Banca') + (banca?.istituto ? ' — ' + banca.istituto : ''), margin, y); y += 5;
  doc.setFontSize(9); doc.setTextColor(107,114,128);
  if (from && to) doc.text('Periodo: ' + formatDate(from) + ' — ' + formatDate(to), margin, y);
  doc.text(currentBusiness?.name || '', W-margin, y, {align:'right'}); y += 10;

  // KPI
  const kpiW = (W - margin*2 - 12)/4;
  const kpis = [
    {label:'Saldo iniziale', val:formatEur(saldoIniziale), color:[37,99,235]},
    {label:'Entrate', val:formatEur(totEntrate), color:[16,185,129]},
    {label:'Uscite', val:formatEur(totUscite), color:[239,68,68]},
    {label:'Saldo finale', val:formatEur(saldoFinale), color:saldoFinale>=0?[16,185,129]:[239,68,68]}
  ];
  kpis.forEach((k,i) => {
    const x = margin + i*(kpiW+4);
  ...
  allMov.forEach((m, i) => {
    ...
    doc.setTextColor(10,15,30); doc.text((m.tipo||'').substring(0,12), margin+24, y+2);
  ...
  saldoFinale>=0 ? doc.setTextColor(52,211,153) : doc.setTextColor(248,113,113);
  doc.text(formatEur(saldoFinale), W-margin-2, y+5.5, {align:'right'});

  doc.setPage(1); doc.setFontSize(7); doc.setTextColor(156,163,175); doc.setFont('helvetica','normal');
  doc.text('KONTRO — Prima nota digitale · www.kontro.cloud', margin, 290);

  const nomeBanca = (banca?.nome||'Banca').replace(/\s/g,'_');
  doc.save(`KONTRO_Banca_${nomeBanca}_${from||'all'}_${to||'all'}.pdf`);
  showToast('PDF estratto conto generato ✓', 'success');
}
...
async function buildFabbisognoFinanziario() {
  ...
  // Ricarica banche fresche dal DB (evita dati stale dopo reset)
  const { data: bancheFresche } = await db.from('banche').select('*')
    .eq('business_id', currentBusiness.id).eq('attivo', true);
  const bancheFabbisogno = bancheFresche || [];
  ...
  // Saldo banche attuale calcolato solo dai movimenti (non da saldo_iniziale stale in cache)
  let saldoBanche = bancheFabbisogno.reduce((s,b) => s + Number(b.saldo_iniziale||0), 0);
  saldoBanche += (vers||[]).reduce((s,v) => s + Number(v.importo_contante||0) + Number(v.importo_pos||0), 0);
  saldoBanche -= (movUsc||[]).reduce((s,m) => s + Number(m.importo||0), 0);
...
async function inviaRicevutaFirma(accontoId) {
  ...
  // Mostra subito un feedback visivo (prima degli await)
  showToast('Generazione link firma...', '');

  // Genera token unico — fallback se crypto.randomUUID non disponibile
  ...
  // Aggiorna il record con il token
  ...
  if (error) {
    // Se fallisce probabilmente mancano le colonne SQL
    showToast('Errore: ' + error.message + ' — hai eseguito il SQL su Supabase?', 'error');
    return;
  }

  // Carica dati per il messaggio
  const { data: acc } = await db.from('acconti_stipendio')
  ...
  const cognome = acc?.dipendenti?.cognome || '';
  const importo = '€ ' + Number(acc?.importo||0).toFixed(2).replace('.', ',');
  const tel     = (acc?.dipendenti?.telefono || '').replace(/\s/g, '').replace('+', '');

  const testo = `Ciao ${nome}, ti inviamo la ricevuta per l'acconto di ${importo}.\nFirma qui (valido 48 ore):\n${link}`;
  const waUrl = tel
    ? `https://wa.me/${tel}?text=${encodeURIComponent(testo)}`
    : `https://wa.me/?text=${encodeURIComponent(testo)}`;

  // Modal firma link — più semplice e affidabile
  const oldM = document.getElementById('modal-firma-link');
  if (oldM) oldM.remove();

  const modal = document.createElement('div');
  modal.id = 'modal-firma-link';
  ...
  modal.innerHTML = `
    <div style="background:#0d1625;border:1px solid rgba(129,140,248,0.5);border-radius:20px;padding:28px;max-width:400px;width:100%">
      <div style="font-size:17px;font-weight:800;color:#c7d2fe;margin-bottom:6px">✍ Ricevuta pronta</div>
      <div style="font-size:12px;color:rgba(255,255,255,.5);margin-bottom:18px">${nome} ${cognome} · ${importo} · valido 48h</div>
      <div style="background:rgba(255,255,255,.05);border-radius:10px;padding:10px 12px;margin-bottom:16px;font-size:11px;color:#60a5fa;word-break:break-all;line-height:1.6">${link}</div>
      <div style="display:flex;flex-direction:column;gap:10px">
        <a href="${waUrl}" target="_blank" rel="noopener noreferrer"
           onclick="setTimeout(()=>document.getElementById('modal-firma-link')?.remove(),500)"
           style="display:flex;align-items:center;justify-content:center;gap:10px;background:linear-gradient(135deg,#075e54,#128c7e);color:white;font-weight:700;font-size:15px;padding:14px;border-radius:12px;text-decoration:none">
        ...
        <button onclick="navigator.clipboard?.writeText('${link}').then(()=>showToast('Copiato ✓','success'))"
          style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.8);font-size:13px;padding:12px;border-radius:12px;cursor:pointer;font-weight:600">
          📋 Copia link
        </button>
        <button onclick="document.getElementById('modal-firma-link').remove()"
          style="background:transparent;border:none;color:rgba(255,255,255,.4);font-size:13px;padding:8px;cursor:pointer">Chiudi</button>
      </div>
    </div>`;
...
async function scaricaRicevutaPDF(accontoId) {
  ...
  } catch(e) { console.warn('Firma PDF error:', e); }
...
async function eseguiReset() {
  ...
  // Azzera saldo_iniziale di tutte le banche (è dato contabile — rappresenta il saldo di partenza)
  await db.from('banche').update({ saldo_iniziale: 0 })
    .eq('business_id', bid);

  // Ricarica bancheCache aggiornata (saldo_iniziale ora = 0)
  if (currentBusiness) {
  ...
  // Azzera immediatamente tutti i widget della dashboard
  const toZero = ['kpi-entrate','kpi-uscite','kpi-saldo','kpi-mese'];
  toZero.forEach(id => { const el = document.getElementById(id); if (el) el.textContent = '€ 0,00'; });
  ['kpi-entrate-count','kpi-uscite-count'].forEach(id => { const el = document.getElementById(id); if (el) el.textContent = '0 movimenti'; });

  const widgetsVuoti = {
    'prev-fabbisogno-content': '<div class="empty-state">Nessuna uscita prevista nei prossimi 30 giorni 🎉</div>',
    'prev-incasso-content':    '<div class="empty-state">Nessun dato storico disponibile</div>',
    'dash-conciliazione':      '',
    'recent-entries':          '<div class="empty-state">Nessun movimento oggi</div>',
  };
  Object.entries(widgetsVuoti).forEach(([id, html]) => {
  ...
  // Ricarica dashboard completa (ora con dati azzerati)
  await loadDashboard();
  showView('dashboard');
}
